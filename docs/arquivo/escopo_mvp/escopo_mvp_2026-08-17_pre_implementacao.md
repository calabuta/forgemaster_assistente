...
comparações; a persistência entre reinicializações ainda não foi definida. O
tipo de arma não muda esse seletor automaticamente: inclusive ao comparar ou
equipar uma arma do outro tipo, a troca de `Melee` para `Ranged` ou vice-versa
continua sendo uma ação manual do usuário. O app calcula sempre pelo modo
selecionado, ignorando o bônus específico do modo inativo; o usuário pode
alternar o seletor para consultar o outro cenário.
